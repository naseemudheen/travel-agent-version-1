...
def get_secret_or_env(key):
    """
    Helper function to get a secret from Streamlit secrets or environment variables.
    Safely handles the case where secrets.toml does not exist.
    """
    try:
...
def get_weather(city: str) -> str:
    ...
    try:
        weather_api_key = get_secret_or_env("OPENWEATHERMAP_API_KEY")
        if weather_api_key:
...
def search_google(query: str) -> str:
    ...
    try:
        serper_api_key = get_secret_or_env("SERPER_API_KEY")
        if serper_api_key:
...
def main():
    ...
    serper_key = get_secret_or_env("SERPER_API_KEY")
    weather_key = get_secret_or_env("OPENWEATHERMAP_API_KEY")
